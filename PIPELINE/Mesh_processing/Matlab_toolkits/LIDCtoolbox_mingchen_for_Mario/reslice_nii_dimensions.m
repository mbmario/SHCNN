...
%
function reslice_nii_dimensions(old_fn, new_fn, voxel_size, verbose, bg, method, img_idx, preferredForm, dimensions)
   ...
   if nii.hdr.dime.dim(5) > 1
      for i = 1:nii.hdr.dime.dim(5)
         ...
         [img(:,:,:,i) M] = ...
		affine_dimensions(nii.img(:,:,:,i), old_M, voxel_size, verbose, bg, method, dimensions);
      end
   else
      [img M] = affine_dimensions(nii.img, old_M, voxel_size, verbose, bg, method, dimensions);
   end
