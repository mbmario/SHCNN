function fileList = find_files(dirName, extension, ignore_dirs, contain_string)
...
mat_ind = cellfun(@(x) strcmpi(x(end-3:end), extension), fileList, 'UniformOutput', true);
% contain the right string
contain_string_ind = logical(true(size(mat_ind)));
if ~isempty(contain_string)
    contain_string_ind = cellfun(@(x) ~isempty(strfind(x, contain_string)), fileList, 'UniformOutput', true); % FIND XML FILES
end


fileList = fileList(mat_ind & contain_string_ind); % keep only XML files
...
for iDir = find(validIndex)                  %# Loop over valid subdirectories
    ...
    fileList = [fileList; find_files(nextDir, extension, ignore_dirs, contain_string)];    %# Recursively call getAllFiles
end
