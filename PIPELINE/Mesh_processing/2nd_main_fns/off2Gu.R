...
# [2] path where Gu files should be saved
{
  ...
  if (!is.na(file.info(args[1])$isdir) && file.info(args[1])$isdir) {
    dir = args[1]   
    cat("Looking for *.off and *_normal.xtx files in directory:", dir, "\n")
  } else {
  ...
  if (!is.na(file.info(args[2])$isdir) && file.info(args[2])$isdir) {
    out_dir = args[2]   
    cat("Will write GU_FORMAT meshes to directory:" , out_dir, "\n") 
  } else {
  ...
  cat("off/normal dir ", dir, "\n", "Gu_format dir dir ", out_dir, "\n")
}

setwd(dir)
#off <- paste(dir, "/off", sep="")
files <- list.files(path=dir, full.names=TRUE, pattern="*.off")
...
for (file in files) {
  ...
  if (vs_n*face_n == 0) {
      next # skip this iteration if either vertices or faces is corrupted
   }
