...
parent_dir <- "/media/mario/DiskB/LIDC_Data_Current/RFData/StaticSets/SH_CNNCombo"
...
appendCNNendPARENT2 <- "/home/mario/Documents/LIDC_Code_Current/PIPELINE/Compile_data/appendCNNendPARENT2.py"  
...
setwd(parent_dir)
set_name <- paste0(SH_include, "SH_CNNcombo")
...
# set df
df <- data.frame()
if (priority_a_bool) { # load the selected df and set it as df
  load(priority_a_path)
  df <- priority_a
}else { 
  load(priority_b_path)
  df <- priority_b
}
...
# args [1] headdir, [2] image_names, [3] features_fc6
    ...
    command <- paste(appendCNNendPARENT2, paste0(parent_dir, "/",set_name), Full_Windows_names, 
                     Full_Windows_csv, Small_Windows_names, Small_Windows_csv)
    system(command)  
